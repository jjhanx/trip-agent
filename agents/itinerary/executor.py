...
- attractions: 최대 {target_n}개 배열 (조건 미달 시 제외시켜 수가 적어도 됨). 각 항목 필수:
  ...
  - practical_details: 객체(모두 한국어, 구체적 수치·절차):
    - parking: **주차·도로(필수)** — **등록 주소·길찾기 주소는 여기에 쓰지 말 것**(중복 방지). **첫머리부터** ① 근처 **가장 가까운 인구 3,000명 이상 거점 도시** 실제 지명 + 인구(명). ② 그 **거점 도심**에서 명소 **입구·주차·트레일 헤드**까지 **승용차 약 ○분**. ③ 주차·톨 €. 예: "거점: 프레데네·산마르티노 다가스트로차 일대(인구 약 5,000명). 거점 시내 기준 Pale di San Martino 입구까지 승용차 약 15분. 주차 €…"
