...
- attractions: 최대 {target_n}개 배열 (조건 미달 시 제외시켜 수가 적어도 됨). 각 항목 필수:
  ...
  - practical_details: 객체(모두 한국어, 구체적 수치·절차):
    - parking: **주차·도로(필수)** — 일정·숙소 판단용 **거점+분**이 본문이다. **한 문단 안에 반드시** (가) **어느 거점 도시(읍)에서** (나) **승용차로 약 몇 분**인지 **숫자+분**으로 쓴다. **내비 검색어 단독 금지**. **등록 주소 금지**. ① **가장 가까운 인구 3,000명 이상** 거점 **지명**+**인구(명)**. ② 그 **거점 도심**에서 **입구·주차·트레일 헤드**까지 **승용차 약 ○분**. ③ 주차·톨 €.
