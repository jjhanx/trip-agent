...
def build_practical_from_details(
    ...
    # 입장·톨 € 수치는 Google 웹 검색 스니펫 + LLM 폴리시로만 채움(여기서는 절차 설명 금지).
    fees = ""
    if website:
        fees = f"공식 웹에서 요금 확인: {website}"
...
def _merge_practical(
    ...
    for k in keys:
        if k == "fees_other":
            if _field_needs_replace(base[k]):
                base[k] = str(fresh.get("fees_other") or "").strip()
            continue
...
- fees_other: **입장료**·환경세·톨·보트·**주차**의 **실제 €·금액·유료·무료**만 **한두 문단**으로 쓴다. **절차·방법·원칙 설명**(예: "…으로 정리한다", "Places API는 …", "스니펫을 모아")은 **절대 쓰지 말 것.** 입력 **`google_web_search_snippets`가 있으면** 그 **Google 웹 검색 스니펫**에서 수치를 반영한다. 도보·하이킹 발췌와 겹치면 통합하고 `웹 검색 기준`·`리뷰 발췌 기준`을 괄호 수준으로만. 미확인 시 "관련 정보 없음".
