...
2. **Amadeus Fallback**: SerpApi 429(한도 초과) 등 **한도 관련** 응답이 감지되고 `AMADEUS_CLIENT_ID`/`AMADEUS_CLIENT_SECRET`이 설정되어 있으면 Amadeus Flight Offers Search API를 호출합니다. **왕복**(`one_way=False`)이면 `_date_pairs_with_flexibility`로 (출발±N, 귀환±N) 쌍을 넘깁니다. **출국 등 편도**(`one_way=True`)는 Amadeus가 `departureDate`만 쓰므로, 예전에는 **(출발일, 귀환일) 한 쌍만** 넣어 **날짜 유연성이 반영되지 않았고**, 현재는 출발일 기준 ±N일·귀환일 이전인 날짜들로 여러 쌍을 만들어 조회합니다(`services._one_way_departure_date_pairs_for_amadeus`). **일반 검색 + `includedAirlineCodes=KE,OZ` 전용 검색**을 병합하며, KE/OZ 보강도 날짜쌍 상위 5개까지 동일하게 맞춥니다.
