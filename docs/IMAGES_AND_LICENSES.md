...
3. **Wikimedia Commons** API: `list=search`로 **여러 검색어**(명소명·목적지·랜드마크 보강어)를 돌린 뒤, **파일 제목마다 명소명과의 점수**를 매겨 상위 후보만 고른다. `prop=imageinfo`가 돌려주는 **`thumburl`(스케일된 JPEG URL)** 만 쓰므로 **고정 경로·오래된 thumb URL**에 의존하지 않는다. 지도·로고류 제목은 제외한다.
...
7. 일정 병합 단계에서 **같은 명소가 두 장**이면 **사진 있는 카드가 대표**가 되도록 점수를 매깁니다(설명만 긴 복제가 이기지 않게 함).
8. 위 과정 후에도 **확실한 매칭이 없으면** `image_url`을 비우고 `image_credit`에 안내합니다. **Unsplash 일반 풍경 폴백은 사용하지 않습니다.**

구현: `shared/place_images.py` (`enrich_attractions_images`, `fetch_commons_unique_thumbnail` 점수·다중 검색).
