...
| 변수 | 설명 |
|------|------|
| AMADEUS_CLIENT_ID | [Amadeus](https://developers.amadeus.com) Client ID (월 2,000회 무료) **권장** |
| AMADEUS_CLIENT_SECRET | Amadeus Client Secret |
| FLIGHTAPI_KEY | [flightapi.io](https://www.flightapi.io) API Key (100회/월 무료) **대안** |
| KIWI_API_KEY | [Kiwi Tequila](https://tequila.kiwi.com) API Key (초대제, 일반 가입 어려움) |
| RAPIDAPI_KEY | RapidAPI Key (Skyscanner API는 현재 RapidAPI에서 제공 중단됨) |

**Amadeus**만 설정해도 항공편 검색 정상 동작. 무료 한도 초과 전 자동 중단·경고 표시. **미설정 또는 결과 없음 시 Mock 데이터 사용**이며, 이때 UI에 **"예시(Mock) 데이터입니다"**가 반드시 표시됩니다.
...
1. **여행 정보 입력**: 출발지·목적지(공항 코드 또는 도시), 출발일·귀환일(달력 버튼으로 날짜 선택), 일행 구성(남·여·아동), 날짜 유연성(±일), 숙소 선호 3순위, 현지 이동, 좌석 클래스, 마일리지 등. 도시 입력 시 8시간 이내 공항 선택 단계가 추가됨
