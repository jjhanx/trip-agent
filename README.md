...
| 변수 | 기본 | 설명 |
|------|------|------|
| GOOGLE_PLACES_API_KEY | | **가장 강력한 명소 사진 API** (Google Maps Platform). 월 $200 무료 크레딧 범위 내에서 우선적으로 고화질 장소 사진을 가져옵니다. ([발급 가이드](docs/GOOGLE_PLACES_API_GUIDE.md)) |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **명소 실무 카드 규칙**: 주차·도로 접근은 **인구 약 3,000명 이상** 최근접 도시(또는 읍·면 거점)와 그곳에서 **명소 입구·주차장·트레일 헤드까지 승용차 몇 분**을 숫자로 명시. **케이블·리프트**는 있을 때만 기재·없으면 빈 칸(UI에서 숨김). **도보·하이킹**은 약 **1000자 전후** 요약(과도한 축약 금지). **입장료·개방·운영 시간·주차 요금·예약 필요**는 필수. Places 보강·LLM 폴리시·오프라인 돌로미티 템플릿·프론트 라벨 반영 (`agents/itinerary/executor.py`, `shared/google_place_details.py`, `frontend/app.js`). |
