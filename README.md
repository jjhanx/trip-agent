...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **돌로미티 명소 검색·병합**: 구글 후보만으로 일수×3칸이 채워지면 오프라인 풀(트레 치메·카디니·라과초이·Sass Pordoi 등)이 뒤로 밀리던 문제를 **우선 시드**로 해소. Nearby에 `cable car`·`funivia`·`Drei Zinnen`, Text Search에 패소·라과초이·토파나 등 쿼리 추가. 돌로미티 구간은 리뷰 수 **품질 필터 완화**. **따뜻한 계절 스키 제외**에 이름 패턴(skitour, Sellaronda 등)과 소개 문구(스키 투어·Sellaronda) 보강(`shared/attraction_filters.py`, `agents/itinerary/executor.py`). |
