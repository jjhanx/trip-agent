...
| TRAVELPAYOUTS_RENTAL_WIDGET_SCRIPT_URL | (선택) Travelpayouts 대시보드에서 발급한 **EconomyBookings Rental Cars Search Form** 스크립트 URL 전체(`https://tpemd.com/content?trs=…`). 렌트 단계에 위젯을 삽입. 앱 픽업·반납 필드와 **자동 연동은 공식 문서 없으면 보장되지 않음** |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-03-23 | 렌트: `TRAVELPAYOUTS_RENTAL_WIDGET_SCRIPT_URL` — 세션이 렌트 단계 JSON에 `travelpayouts_rental_widget_script_url`을 실어 주고, 프론트가 **EconomyBookings 검색 위젯**(tpemd 스크립트)을 마운트. 문서·`.env.example` 반영 |
