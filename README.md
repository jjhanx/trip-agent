...
| AMADEUS_CLIENT_ID / AMADEUS_CLIENT_SECRET | 항공과 **동일 키**. 렌트 단계에서 Transfer API로 **공항↔시내 트랜스퍼** 견적(1회 구간) 조회. 셀프 드라이브 총액은 별도 상품이라 EconomyBookings 링크로 안내 |
...
3. **렌트카/대중교통**: 렌트카 선택 시 **픽업·반납 일시·공항(IATA)** 입력란에 항공 일정이 채워지며 수정 후 **이 일정으로 다시 검색** 가능. `AMADEUS_*` 설정 시 **공항↔시내 트랜스퍼 1회 구간** 견적(차종·원화 환산 표시)을 우선 표시하고, **셀프 드라이브 기간 총액**은 EconomyBookings 링크에서 확인. 대중교통은 Transit MCP(노선·체류일 패스)
