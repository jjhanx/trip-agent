...
**SerpApi**는 월 250회 무료. 미설정 또는 한도 초과 시 Playwright 크롤링으로 전환되며, 이마저 실패하면 **Mock 데이터** 사용. UI에 **"예시(Mock) 데이터입니다"**가 표시됩니다.
...
**계획 저장·불러오기 (파일처럼 편집)**: 여행 계획은 단계별로 시간이 걸리므로, 상단 **저장** / **다른 이름으로** / **열기** 버튼으로 진행 상황을 저장하고 나중에 다시 불러와 이어서 편집할 수 있습니다. 매번 검색을 다시 하지 않고 계속 계획을 이어갈 수 있습니다. (브라우저 localStorage에 최대 50개까지 저장)
