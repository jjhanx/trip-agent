...
5. **숙소 선택**: 일정·일행·선호 숙소 형태를 반영한 **최대 5개** 혼합 후보(조식·주방·침실·주차·부대시설·대표 사진·예약 URL·동선·케이블카/주차 참고). 하단에 **현지 이동**은 렌트/교통 카드 형태로 요약
6. **일정 확정** → 예약 절차 안내
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-16 | **숙소 단계 UI·목 데이터**: 세션이 일행 수·선택 일정을 숙소 에이전트·Hotel MCP에 넘기고, 목 후보에 조식·주방·침실·주차·수영장·자쿠지·사우나 등 요약·대표 사진(최대 6)·Booking 검색 URL·선정 근거·케이블카/주차 참고 문구를 포함. **현지 이동**은 JSON `<pre>` 대신 렌트 단계와 동일한 **읽기 전용 카드**로 표시. `app.js?v=36`, `styles.css?v=13`, `agents/session/executor.py`, `agents/accommodation/executor.py`, `mcp_servers/hotel/`. |
