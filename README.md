...
5. **숙소 선택**: `GOOGLE_PLACES_API_KEY`·Distance Matrix 등 설정 시 **일정 명소 좌표** 기준으로 숙소 후보를 고르고 **숙소→명소 승용차 편도 분**을 표시([docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md)). 키가 없으면 mock. **렌트카**를 고른 경우 이 단계에서는 **현지 이동** 목록을 비우고 주행 정보는 숙소 카드에만 표시
6. **일정 확정** → 예약 절차 안내
