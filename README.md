...
**SerpApi**는 월 250회 무료. 미설정 또는 한도 초과 시 **Mock 데이터**로 대체. 한도 초과 시 UI에 점검 방법(dashboard, Account API)이 표시되며, 자세한 점검 절차는 [FLIGHT_API_SETUP.md §4](FLIGHT_API_SETUP.md#4-serpapi-한도-초과-시-점검-방법) 참조.
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2025-03-17 | 왕복+날짜유연성: 출발일±N·귀환일±N 편도 각각 검색 후 조합, 가격 싼 순. 0건 시 실제 날짜범위로 안내(출발일 OO부터 OO까지와 귀환일…) |
