...
| 변수 | 설명 |
|------|------|
| DUFFEL_ACCESS_TOKEN | [Duffel](https://duffel.com) Live Access Token (대한항공·아시아나 포함 300+ 항공사) |

**Duffel**은 검색 무료, 예약 시에만 비용 발생. **미설정 또는 결과 없음 시 Mock 데이터 사용**이며, 이때 UI에 **"예시(Mock) 데이터입니다"**가 반드시 표시됩니다. **Live Access Token**을 사용해야 대한항공·아시아나 등 실제 운항편이 조회됩니다 (테스트 토큰은 Duffel Airways만 반환).
...
2. **다음** → 항공편 결과: 추천순(선호 직항→선호 경유→나머지 직항→나머지 경유, 각 그룹 내 비행시간·가격순) 또는 가격순·비행시간순 선택
...
| 2025-03-03 | 문서 정리: DEPLOYMENT에 DUFFEL_ACCESS_TOKEN 안내, README 사용 흐름·추천순 설명, 서버 배포 체크리스트 |
