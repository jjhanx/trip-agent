...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **명소 실무·후보 품질**: 주차 **인구 3천·차로 몇 분**은 LLM 프롬프트에만 두고, **Places `build_practical_from_details`**는 주소·검색·운영시간·리뷰 발췌 등 **사용자용 문장만** 출력. **Tre Cime** 등 이름 중복 병합, **사진** Details·Commons·미확보 시 제외, 플레이스홀더 마커 확장 (`agents/itinerary/executor.py`, `shared/place_images.py`, `shared/google_place_details.py`, `frontend/app.js`). |
