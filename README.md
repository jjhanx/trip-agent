...
| A2A_ITINERARY_TIMEOUT_SECONDS | 600 | Session → **Itinerary** A2A 타임아웃(초). 명소·LLM·이미지 보강이 길면 늘림. **Nginx·로드밸런서** `proxy_read_timeout`(또는 idle timeout)도 이 값 이상 권장(HTTP 504 완화). |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **목적지 파타고니아인데 MXP**: 미등록 도시는 공항 선택 단계가 생략되는데 `localStorage`에 남은 `destination_airport_code`(돌로미티 등)가 그대로 전송됨. 초기 로드·목적지 입력 변경·**저장 계획 불러오기** 후에도 검증·초기화(`validateDestinationAirportMatchesDestination`, `loadPlanIntoState`, `initDestinationAirportSync`). `findCityKey`에서 `k.includes(t)` 제거(짧은 부분 문자열 오매칭 방지). `airports.js`에 **파타고니아/Patagonia** 공항 후보(FTE·USH·EZE·PMC·SCL) 추가. `app.js?v=14`. |
