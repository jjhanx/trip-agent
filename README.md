...
│   ├── hotel/                 # search_hotels(Places+Distance Matrix), compare_hotels
...
| 변수 | 기본 | 설명 |
|------|------|------|
| GOOGLE_PLACES_API_KEY | | **명소 사진·Place Details·좌표** (Google Maps Platform). 동일 키로 **Directions API**(거점→명소 주행 분)·**Geocoding API**(목적지·거점 좌표)·**맛집 단계 Nearby Search**(명소 주변 식당)·**Distance Matrix API**(숙소→명소 승용차 분 합산)를 씁니다. Cloud Console에서 **Places, Directions, Geocoding, Distance Matrix**가 사용(Enabled)되어 있어야 합니다. 숙소 실검색 설정은 [docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md). 월 $200 무료 크레딧 범위 참고. ([발급 가이드](docs/GOOGLE_PLACES_API_GUIDE.md)) |
...
5. **숙소 선택**: `GOOGLE_PLACES_API_KEY`·Distance Matrix 등 설정 시 **날짜별 방문 명소**만 두고 숙소 후보·**숙소→이날 명소 승용차 편도 분**을 표시하고, Travelpayouts 토큰이 있으면 **일행·객실 수 반영 요금(추정)** 을 카드·하단 합산에 반영([docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md)). 키가 없으면 mock. **렌트카**를 고른 경우 이 단계에서는 **현지 이동** 목록을 비우고 주행 정보는 숙소 카드에만 표시
6. **일정 확정** → 예약 절차 안내
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-17 | **예약 체크리스트가 안 그려지던 논리 버그**: `refreshStepConfirmSection`이 `bookingGuidanceReceived === true`일 때만 `renderBookingGuidanceIntoDom`을 호출해, 저장본에 `bookingGuidanceData`만 있고 `received`가 false이면 **카드 미표시**·`renderStepConfirmFromItinerary`가 **안내 패널까지 지움**. 확정 화면에서는 **항상** `ensureBookingGuidanceSteps`로 체크리스트를 그리고, 안내 유지 여부는 `shouldPreserveBookingGuidancePanel()`로 판단. 일정 완료·확정 탭 진입 등 `show('step-confirm', true)`로 새로고침 경로 보강. `app.js?v=54`. |
| 2026-04-17 | **예약 안내: 단계 카드가 계속 비어 보이던 원인(레거시 분기)**: 예전 분기가 `#booking-guidance` **전체**를 한 번의 `innerHTML`로 덮어 **`#booking-guidance-lead`·`#booking-guidance-steps` 자식이 사라지면** 이후에는 그 경로만 반복되어 요약만 보이고 카드가 없음. 렌더 전 자식이 없으면 **div 두 개를 다시 붙이고**(`ensureBookingGuidanceDomStructure`), 단계는 **`<section>`마다 별도 `innerHTML`**. `app.js?v=53`, `index.html`. |
| 2026-04-17 | **예약 안내 DOM 분리**: 요약(`#booking-guidance-lead`)과 단계 카드(`#booking-guidance-steps`)를 **서로 다른 `innerHTML`**로 갱신해, 항공 등 한 구역의 마크업이 깨져도 나머지 단계가 사라지지 않게 함. `styles.css?v=25`, `app.js?v=52`, `index.html`. |
| 2026-04-17 | **예약 안내: 요약만 보이고 단계 카드가 사라지던 현상**: `formatFlightDetailForSummary`가 항공 API 문자열을 **HTML 이스케이프 없이** `innerHTML`에 넣어, 특정 문자가 포함되면 **한 블록 안의 뒤쪽 DOM(항공·숙소·일정 카드)** 이 브라우저 HTML 파서에서 깨질 수 있음. 항공 요약 카드 전 필드 `escapeHtml` 처리. `app.js?v=51`. |
| 2026-04-17 | **예약 안내 카드가 안 보이던 문제**: 서버 `steps`를 그대로 쓰면 본문이 비는 경우가 있어, **항공·숙소·일정 3단계는 항상 `state`에서 생성**하고 서버는 `action` 문구만 항목명으로 매칭해 덮어씀. 단계별 본문 렌더 **try/catch**. `data.error` 표시 조건 정리. `app.js?v=50`. |
| 2026-04-17 | **예약 안내 보강·버튼**: 응답에 `steps`가 없거나 중첩 키일 때 **현재 상태로 체크리스트 보강**(`ensureBookingGuidanceSteps`). 서버 안내를 받은 뒤에만 버튼을 「안내 다시 받기」로 바꿈(`bookingGuidanceApiReceived`). 9단계 진입 시에도 로컬 체크리스트 표시. 안내 문구·`step-confirm-actions` flex 정리. `app.js?v=49`, `styles.css?v=24`, `index.html`. |
| 2026-04-17 | **예약 안내 UI**: 세션·예약 에이전트 응답을 `<pre>` JSON 덤프 대신 **요약 문단 + 단계별 카드**(항공·숙소·일정)로 표시. 항공은 기존 선택 카드 스타일, 숙소는 거점·일자별 목록·추정 합계·예약 링크, 일정은 제목·요약·일자별 표. `bookingGuidanceData`를 저장·복원해 계획 불러오기 후에도 동일하게 표시. `app.js?v=48`, `styles.css?v=23`. |
| 2026-04-17 | **숙소 선택 첫 진입 시 목록 비어 보임**: 동선 계획에서 숙소로 넘어갈 때 `renderAccommodations`가 로딩·숨김 상태에서만 실행되면 첫 페인트가 비는 경우가 있어, `refreshAccommodationPanel()`을 두고 `show('step-accommodation')` 시 즉시·`requestAnimationFrame`으로 다시 그리도록 함. `app.js?v=47`. |
| 2026-04-17 | **동선 계획 빈 화면(「일정 1」만)**: 응답에 `route_plan`은 있으나 `itinerary_step`이 빠지면 레거시 일정 카드 분기로 가던 문제를 프론트 `normalizeRouteBundleIfMissingStep`·`raw` JSON 재파싱으로 복구. 서버 `route_restaurants` 응답 전송 직전 `itinerary_step`·`trip_dates` 보장. `app.js?v=46`, `agents/itinerary/executor.py`. |
| 2026-04-17 | **숙소 실용 정보(Hotellook·UI)**: `hotellook_prices.fetch_hotellook_stay_quote`로 캐시 JSON에서 요금 노드를 모아 **조식·식사 포함으로 보이는 옵션 중 최저가**를 우선(없으면 전체 최저가), 객실·식사 텍스트·합산 원화·재고 면책 문구·제휴 링크를 숙소 객체에 반영. Places 카드 문구에서 불필요한 「공식 사이트만 확인」 톤을 줄이고 주차 키워드 추정을 세분화. 프론트에 `meal_plan_summary_ko`, `availability_note_ko` 표시·예약 버튼 라벨 정리. `docs/HOTEL_SEARCH_API.md`에 토큰·캐시 한계 명시. `app.js?v=45`, `mcp_servers/hotel/hotellook_prices.py` 등. |
| 2026-04-17 | **숙소 선택 레이아웃**: 「이 숙소 선택」을 카드 **상단 한 줄**로 옮겨 가로 폭을 덜 쓰게 함. 썸네일은 **한 줄 3장·최대 두 줄(6장)** 그리드로 카드 폭에 맞춤(`grid-template-columns: repeat(3, minmax(0,1fr))`, `aspect-ratio: 4/3`). `styles.css?v=22`. |
| 2026-04-17 | **숙소 선택 UI**: 거점·일자별 숙소 후보 사진을 **썸네일(88×66)** 로 표시하고 클릭 시 **라이트박스**로 확대. 구간 헤더·날짜·동선 텍스트 **줄바꿈·폭 제한**으로 카드 밖으로 삐져나가지 않게 함. 카드 왼쪽 **체크박스「이 숙소 선택」** 으로 확정(구간·날짜당 하나만 선택)하고 기존 `accommodationSelectionByGroup` / `accommodationSelectionByDate` / `selectedAccommodation` 에 반영. `app.js?v=44`, `styles.css?v=21`, `index.html`(라이트박스 마크업). |
| 2026-04-16 | **일정 9단계 UI(동선·숙소·맛집·확정·예약 분리)**: 상단 칩을 명소 선택→동선 계획→숙소 선택→맛집 선택→일정 확정→예약 안내로 맞추고, `syncStepIndicatorActive`·`navigateToStep`·`pick_lodging` 워크플로로 단계별 이동·저장 상태 복원. 동선 화면에서 하단 「맛집 우선순위」중복 제거, 숙소 후 「다음 (맛집 선택)」, 확정 후 `step-confirm`·「예약 안내 받기」(`btn-request-booking-guidance`). 동선만 반영한 일정은 `preview_only`로 확정 여부 판별에서 제외. `app.js?v=43`, `styles.css?v=20`. |
| 2026-04-16 | **숙소 빈 화면·경로/맛집 하위 단계**: 숙소 에이전트가 세그먼트 응답을 `[:5]`로 잘라 목록이 깨지던 문제 수정. 거점 구간 숙소가 전부 비면 일자별·mock으로 폴백. 확정 일정에 `route_plan`이 없으면 `itineraryRouteBundle`으로 보강 후 숙소 MCP 호출. 경로·지도와 맛집 선택을 하위 단계로 나누고 초안·계획 저장에 `itineraryMealsUiPhase` 반영. 숙소 목록 없을 때 안내·재시도 버튼. `app.js?v=42`, `styles.css?v=19`. |
| 2026-04-16 | **여행 일정 지도·숙소 거점 그룹**: 경로·맛집 단계에서 `route_plan.map_payload`로 **Leaflet**(OSM)에 루프 polyline·**날짜별 색 마커**·툴팁. `stay_groups` 범례(같은 숙소로 묶인 일자). 숙소 MCP는 **`stay_group_hint`(구간별 거점)** 검색을 **우선**하고, 실패 시 일자별 `daily_stay_hint`로 폴백. 계획 저장에 `accommodationSelectionByGroup`·확정 시 `selected_accommodation.mode: by_stay_group`. `frontend/index.html`(Leaflet CDN), `app.js?v=41`, `styles.css?v=18`; `shared/route_map_payload.py`, `mcp_servers/hotel/services.py`, `mcp_servers/hotel/google_places_hotels.py`. |
| 2026-04-16 | **동선·숙소**: **Distance Matrix + NN·2-opt**로 전체 명소 투어 순서(직선 사영 제거)·`loop_route` 지도. 일별 숙소 **반경 확대·후보 14곳 스코어**·60분 우선·실패 시에도 **날짜 블록 유지**. [docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md). `shared/tour_route_optimizer.py`. `app.js?v=40`, `styles.css?v=17`. |
