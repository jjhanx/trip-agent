...
| SERPAPI_API_KEY | [SerpApi](https://serpapi.com) — **항공 검색 1순위** (Google Flights). 일정 명소 **입장료·주차비** 보강에도 사용(명소당 Google 검색 2회·스니펫 → LLM `fees_other`) |
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **입장료·주차 SerpApi 검색**: `SERPAPI_API_KEY`가 있으면 명소마다 `「이름 입장료」`·`「이름 주차비」`(짧은 이름은 목적지 포함) Google 검색 스니펫을 모아 LLM 폴리시가 `fees_other`에 수치·€로 정리. OpenAI 없을 때는 스니펫을 그대로 붙임. `shared/fees_web_search.py` (`polish_practical_details_with_llm`, `postprocess_attraction_list_for_catalog`). |
