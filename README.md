...
| 변수 | 설명 |
|------|------|
| FLIGHTAPI_KEY | [flightapi.io](https://www.flightapi.io) API Key (100회/월 무료) **권장** |
| KIWI_API_KEY | [Kiwi Tequila](https://tequila.kiwi.com) API Key (초대제, 일반 가입 어려움) |
| RAPIDAPI_KEY | RapidAPI Key (항공편 API 플랜별) |

**flightapi.io**만 설정해도 항공편 검색 정상 동작. 무료 한도 초과 전 자동 중단·경고 표시. **미설정 또는 결과 없음 시 Mock 데이터 사용**이며, 이때 UI에 **"예시(Mock) 데이터입니다"**가 반드시 표시됩니다.
