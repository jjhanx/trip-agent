...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **parking 문구 간결화**: `(가) 거점 … (나) …` 이중 문장 대신 **`{거점}에서 이 명소까지 승용차 약 N분 (Google Maps 도로 검색 기준).`** 한 줄로 표시. LLM 구조 필드 조합·`parking_requires` 판정 동일 형식 반영 (`shared/directions_parking.py`, `shared/google_place_details.py`). |
