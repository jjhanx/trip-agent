...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **명소 실무·후보 품질**: Places 보강은 사용자용 문만; **주차**는 **최근접 인구 3천 이상 도시·인구·승용차 ○분** 검증 후 미달 시 **parking 전용 재폴리시** 1회 (`parking_meets_nearest_city_pop3000_and_drive_minutes`, `shared/google_place_details.py`). Tre Cime 등 **이름 중복 병합**, **사진** Details·Commons·미확보 시 제외 (`agents/itinerary/executor.py`, `shared/place_images.py`, `frontend/app.js`). |
