...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **명소 실무·후보 품질**: 주차에 **인구 3천 이상 도시명·차로 몇 분** 예시 포함해 프롬프트 강화. **Tre Cime** 등 괄호 별칭 제거 후 **이름 중복 병합**. **사진**: `place_id`만 있을 때 **Places Details `photos`** 보강·**Commons 키워드 폴백**·없으면 후보 **제외**. 프롬프트 **지시문 노출**은 기본 실무 블록 제거·메타 문구 삭제·플레이스홀더 마커 확장 (`agents/itinerary/executor.py`, `shared/place_images.py`, `shared/google_place_details.py`, `frontend/app.js`). |
