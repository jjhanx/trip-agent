...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **명소 UI**: 소개 본문에 `google_maps_url`과 동일한 URL이 있을 때, 그 **URL 직후** 중복 라벨 `Google Maps` 한 번만 제거(문장 안 다른 표현은 유지). `입장료·톨·기타`는 `http(s)://` URL을 새 탭 링크로 표시(`renderPracticalDetailsHtml`, `frontend/styles.css`). |
