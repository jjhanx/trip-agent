...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **UI: 단계 진행/건너뛰기**: 여행 정보 입력 후 **항공편 검색·선택** 또는 **항공편 건너뛰기**를 고르는 화면(`step-input-bridge`). 이후에도 항공·렌트·일정·숙소 단계마다 **다음**과 함께 **건너뛰기** 버튼 제공. 세션 에이전트는 `flight_skipped`·`rental_skipped`로 항공 없이 렌트/일정으로 이어지게 처리(`agents/session/executor.py`). `frontend/app.js?v=17`. |
