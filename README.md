...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-16 | **숙소**: `daily_schedule`이 있으면 **날짜별** 명소만 Distance Matrix에 넣어 후보(일당 최대 3)·`drive_time_scope`·Hotellook(객실 수)·프론트 **일자별 섹션·날짜마다 선택·요금 합산 추정**·확정 `selected_accommodation.mode=by_date`. 일별 수집 실패 시 **전체 명소 레거시** 폴백(주행 분은 “전체 일정 기준”으로 표시). [docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md). `app.js?v=38`, `styles.css?v=15`. |
