...
| SERPAPI_API_KEY | [SerpApi](https://serpapi.com) — **항공 검색 1순위** (Google Flights) |
...
### 6.2.1 렌트카 (Amadeus 트랜스퍼 견적 + Travelpayouts 제휴, 선택)

| 변수 | 설명 |
|------|------|
| TRAVELPAYOUTS_RENTAL_BOOKING_URL | 대시보드에서 생성한 렌트카 제휴 검색 URL. 설정 시 목록에 **제휴** 카드로 포함 |
| AMADEUS_CLIENT_ID / AMADEUS_CLIENT_SECRET | 항공과 **동일 키**. 렌트 단계에서 Transfer API로 **공항↔시내 트랜스퍼** 견적(1회 구간) 조회 |
| SERPAPI_API_KEY | 항공과 **동일 키**. 렌트 단계에서 **Google 검색 organic** 결과로 셀프 드라이브 OTA·비교 사이트 후보를 만들고, 스니펫에서 **가격 힌트(추정)** 를 붙임. 최종 요금은 링크에서 확인 |
...
3. **렌트카/대중교통**: 렌트카 선택 시 **픽업·반납 일시·공항(IATA)** 입력란에 항공 일정이 채워지며 수정 후 **이 일정으로 다시 검색** 가능. `SERPAPI_API_KEY`가 있으면 **Google 검색 결과**를 바탕으로 셀프 드라이브 **후보 사이트·추정 가격 힌트·출처 링크** 카드를 붙입니다. `AMADEUS_*`가 있으면 **공항↔시내 트랜스퍼 1회** 견적을 추가로 표시합니다. EconomyBookings 등 비교 링크는 유지. 대중교통은 Transit MCP
