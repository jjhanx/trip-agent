...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-16 | **동선·숙소**: 도착 앵커에서 **최근접·최원거리** 명소를 고르고 **앵커→근접→원거리→앵커** 루프(`loop_route`: Directions·정적 지도·Maps 링크). 나머지 명소는 주 구간에 **사영 정렬** 후 **하루 2곳**·전일 대비 구역 이동이 길면 **숙소 이동 검토** 플래그. 숙소: 일별 Distance Matrix·**편도 60분 우선**·Places **website**·시설 키워드 추정·일자별 UI·`by_date` 확정. [docs/HOTEL_SEARCH_API.md](docs/HOTEL_SEARCH_API.md). `shared/loop_route_planner.py`. `app.js?v=39`, `styles.css?v=16`. |
