...
| 변수 | 설명 |
|------|------|
| TRAVELPAYOUTS_RENTAL_BOOKING_URL | 대시보드에서 생성한 렌트카 제휴 검색 URL. 설정 시 목록 **하단**에 **제휴** 카드 |
| AMADEUS_CLIENT_ID / AMADEUS_CLIENT_SECRET | **항공**용(SerpApi 429 시 fallback). 렌트 단계에서는 트랜스퍼를 호출하지 않음 |
| SERPAPI_API_KEY | 항공과 **동일 키**. 렌트 단계에서 **Google 검색**(영문·현지어 병합)으로 셀프 드라이브 후보·스니펫 **가격 힌트(추정)**. 차급별 스펙 참고 카드(`vehicle_class_guide`)는 동일 EconomyBookings URL로 실시간 총액 확인 유도 |
