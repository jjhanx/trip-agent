...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-04-02 | **Grand Circle 명소**: 출발지에서 **LAX / LAS / PHX / SLC** 중 대원거리 최단을 게이트웨이로 선택하고, Google Directions **루프**(그랜드캐년·자이언·브라이스·페이지·모뉴먼트 밸리·아치스) 샘플로 주변만 Places 검색; 루프 앵커까지 **약 110km**·평점·리뷰 수 정렬. 해외 출발지 앵커를 쓰지 않아 시카고 등 오탐을 줄임. `shared/grand_circle_places.py`. [docs/ATTRACTION_PIPELINE.md](docs/ATTRACTION_PIPELINE.md). `frontend/airports.js?v=9`. |
