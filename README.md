...
| 변수 | 설명 |
|------|------|
| SERPAPI_API_KEY | [SerpApi](https://serpapi.com) API Key (Google Flights 연동) |

**SerpApi**는 구글 항공권 검색엔진을 활용하여 매월 250건까지 무료로 대한항공·아시아나 등 전 세계의 각종 항공편의 실제 정보와 가격을 반환합니다. 제공된 무료 횟수를 모두 소진하거나 에이전트 내부에 오류가 발생할 시 자동으로 **Playwright 브라우저 크롤링 기능**을 작동시켜 스크래핑을 수행합니다. 위 2가지 방법이 모두 불가할 때는 예비용(Mock) 데이터로 진행되며 UI에 **"예시(Mock) 데이터입니다"**가 명시됩니다.
...
1. **여행 정보 입력**: 여정 유형(왕복, 편도, 다구간), 출발지·목적지(공항 코드 또는 도시), 출발일·귀환일(달력 버튼으로 날짜 선택), 일행 구성(남·여·아동), 날짜 유연성(±일), 숙소 선호 3순위, 현지 이동, 좌석 클래스, 마일리지·마일리지 프로그램 등. 도시 입력 시 8시간 이내 공항 선택 단계가 추가됨
2. **다음** → 항공편 결과: 가격순 또는 마일리지순 확인 후 선택
3. **다음** → 일정 선택: 3개 일정안 중 선택 (조기 도착 우선 고려)
4. **다음** → 숙소 선택: 5개 혼합 후보 중 선택 (현지 이동 정보 포함)
5. **일정 확정** → 예약 절차 안내
...
| 날짜 | 변경 내용 |
|------|-----------|
| 2026-03-11 | 항공편 검색 여정 유형(왕복, 편도, 다구간) 선택 추가 및 SerpApi type 파라미터 연동, 디버그 모드 추가 |
...
| 2025-03-03 | Flight API Duffel 전환: flightapi/Kiwi/RapidAPI 제거, Duffel 단독 사용 (대한항공·아시아나 실제 데이터) |
